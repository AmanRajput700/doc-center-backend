...
router.post('/upload', verifyToken, upload.single('document'), asyncHandler(async function _upload(req, res, next) {
...
router.get('/', verifyToken, asyncHandler(async function _getDocs(req, res, next) {
    const dbName = req.tenant.dbName;
...
router.delete('/:id', verifyToken, checkRole('Admin'), asyncHandler(async function _deleteDocument(req, res, next) {
...
router.put('/:id', verifyToken, checkRole('Admin'), asyncHandler(async function _updateDocument(req, res, next) {
    const docId = req.params.id;
    const data = req.body;
