...
module.exports = async function (page, limit, dbName) {
    const skip = (page - 1) * limit;
    const User = getTenantModel(dbName, 'User', userSchema);
    const Role = getTenantModel(dbName, 'Role', roleSchema);
    const totalUsers = await User.countDocuments();
    const users = await User.find({}, { _id: 1, firstName: 1, lastName: 1, email: 1, role: 1, status: 1, lastLogin: 1, lastActivateAt: 1 })
        .populate('role', 'name')
        .skip(skip)
        .limit(limit)
        .lean();
