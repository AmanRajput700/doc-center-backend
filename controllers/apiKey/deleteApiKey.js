...
module.exports = async function (apiKeyId, tenant, userId) {
    ...
    if (!user || !user.role || user.role.name !== 'Admin') throw new createHttpError(STATUS_CODE.FORBIDDEN, 'User not allowed to Delete API key');

    const apiKey = await ApiKey.findById(apiKeyId);
    if (!apiKey) throw new createHttpError(STATUS_CODE.NOT_FOUND, ERROR_MESSAGE.API_KEY_NOT_FOUND);
